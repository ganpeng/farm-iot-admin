  ...
  <div class="sensor-table-container">
    <el-table
      ...
      <el-table-column
        ...
        label="厂商名称">
        <template slot-scope="scope">
          <div class="table-item">{{scope.row.controllerVendor}}</div>
        </template>
      </el-table-column>
      <el-table-column
        prop="farmName"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <input
            ...
            @click.prevent="updateControllerReceive(scope.row)"/>
        </template>
      </el-table-column>
      -->
        ...
        <template slot-scope="scope">
          <div class="operator-btn-wrapper">
              <span class="btn-text"
                    ...
                    @click="resetController(scope.row)">重启</span>
            ...
            <span :class="{disabled:!(!scope.row.receive || scope.row.deviceStatus === 'UNACTIVE')}"
                  class="btn-text text-danger" @click="removeController(scope.row)">删除</span>
          </div>
        </template>
      </el-table-column>
    </el-table>
    ...
    methods: {
      ...
      updateControllerReceive(item) {
        ...
        this.$confirm('此操作将' + (!!item.receive ? '拒收' : '接收') + (item.name || '当前') + '设备, 是否继续?', '提示', {
          ...
          this.$service.updateControllerReceive({
            deviceName: item.deviceName,
            id: item.id,
      ...
      removeController(controller) {
        ...
        if (!(!controller.receive || controller.deviceStatus === 'UNACTIVE')) {
        ...
        this.$confirm('<div class="message-notice-block"><div>确定删除该设备？</div><div>删除后将不能恢复，请谨慎操作。</div></div>', '提示', {
          ...
          this.$service.removeController(controller.id).then((res) => {
            if (res && res.code === 0) {
              this.$message.success('控制设备删除成功');
              this.$emit('getControllerList');
            }
          });
        });
      },
      ...
      batchExport() {
        ...
        for (let i = 0; i < this.multipleSelection.length; i++) {
          ...
          list.push({
            ...
            '地块名称': obj.landName,
            '设备名称': obj.name,
            '控制设备类型': obj.controllerType || '/',
            '设备位置（经纬度）': (obj.latitude || '') + '/' + (obj.longitude || ''),
            '厂商': obj.controllerVendor,
        ...
        XLSX.writeFile(wb, '控制设备导出表_' + this.$util.formatDate(new Date(), 'yyyy-MM-DD_HH:mm:SS') + '.xlsx');
      }
    }
  };
</script>
