        ...
        <el-row>
            ...
            <el-col :span="12">
                <el-form-item label="姓名" prop="name">
                    <el-input
                        :value="plantMember.name"
                        @input="inputHandler($event, 'name')">
                    </el-input>
                </el-form-item>
                <el-form-item label="身份证号" prop="identityId">
                    <el-input
                        :value="plantMember.identityId"
                        @input="inputHandler($event, 'identityId')">
                    </el-input>
                </el-form-item>
                <el-form-item label="贫困户" prop="isPoor">
                  <el-radio @input="inputHandler(true, 'isPoor')" :value="plantMember.isPoor" :label="true">是</el-radio>
                  <el-radio @input="inputHandler(false, 'isPoor')" :value="plantMember.isPoor" :label="false">否</el-radio>
                </el-form-item>
            </el-col>
            <el-col :span="12">
                <el-form-item label="联系电话" prop="mobile">
                  <el-input
                    :value="plantMember.mobile"
                    type="number"
                    @input="inputHandler($event, 'mobile')">
                  </el-input>
                </el-form-item>
                <el-form-item label="性别" prop="gender">
                  <el-radio @input="inputHandler('MALE', 'gender')" :value="plantMember.gender" label="MALE">男</el-radio>
                  <el-radio @input="inputHandler('FEMALE', 'gender')" :value="plantMember.gender" label="FEMALE">女</el-radio>
                </el-form-item>
            </el-col>
            <el-col :span="24">
                  ...
                  <el-col :span="12">
                      ...
                      >
                        <file-carousel
                          ...
                          :deleteFile="deleteFileHandler(index, 'trainingList')" :files="training.url"></file-carousel>
                        <uploader v-if="training.url.length < 5" :id="`trainingFileUploader${index}`" :uploadSuccessHandler="fileUploadHandler(index, 'trainingList')"></uploader>
                      </el-form-item>
                  </el-col>
                </div>
              </li>
                  ...
                  <el-col :span="12">
                    ...
                    >
                      <file-carousel
                        ...
                        :deleteFile="deleteFileHandler(index, 'certificateList')" :files="certificate.url"></file-carousel>
                      <uploader v-if="certificate.url.length < 5" :id="`certificateFileUploader${index}`" :uploadSuccessHandler="fileUploadHandler(index, 'certificateList')"></uploader>
                    </el-form-item>
                  </el-col>
                </div>
              </li>
...
export default {
  ...
  data() {
    return {
      ...
      validRules: {
        ...
        isPoor: [{required: true, message: "请选择是否为贫困户"}],
  ...
  methods: {
    ...mapMutations({
      updateCurrentPlantMember: "plantMember/updateCurrentPlantMember",
      addTraining: "plantMember/addTraining",
      deleteTrainingByIndex: "plantMember/deleteTrainingByIndex",
      addCertificate: "plantMember/addCertificate",
      deleteCertificateByIndex: "plantMember/deleteCertificateByIndex",
      updateExpandedByKeyIndex: "plantMember/updateExpandedByKeyIndex"
    }),
    ...mapActions({
      createPlantMember: 'plantMember/createPlantMember',
      updatePlantMemberById: 'plantMember/updatePlantMemberById',
      getPlantMemberList: 'plantMember/getPlantMemberList',
      approvePlantMemberSubmit: 'plantMember/approvePlantMemberSubmit',
    ...
    inputHandler(value, key) {
      this.updateCurrentPlantMember({key, value});
    },
    // 农场主角色 填写更新内容
    editContent(value) {
      this.updateCurrentPlantMember({key: 'updateInfo', value})
    ...
    async openDialogHandler() {
      try {
        ...
        this.$refs.plantMemberForm.clearValidate();
      } catch (err) {
        console.log(err);
      }
    },
    ...
    setAddress() {
      ...
      if (address || areaCode) {
        if (_.get(this.plantMember, 'address') !== address
          || _.get(this.plantMember, 'areaCode') !== areaCode) {
          this.updateCurrentPlantMember({key: 'address', value: address});
          this.updateCurrentPlantMember({key: 'areaCode', value: areaCode});
          this.updateCurrentPlantMember({key: 'detailAddress', value: detailAddress || ''});
        }
      }
    },
    ...
    deleteFileHandler(index, key) {
      return (_index) => {
        let url = _.get(this.plantMember, `${key}.${index}.url`);
        let urlList = url.filter((item, __index) => _index !== __index);
        this.updateCurrentPlantMember({key: `${key}.${index}.url`, value: urlList});
      }
    },
    ...
    async enterHandler() {
      try {
        ...
        let valid = await this.$refs.plantMemberForm.validate();
        if (valid) {
          ...
          let res = await this.approveFarmEditSubmit({id: this.checkId, approveInfo: this.plantMember, updateInfo: this.updateInfo});
          if (res && res.code === 0) {
