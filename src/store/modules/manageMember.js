...
const state = {
  ...
  currentManageMember: _.cloneDeep(defaultManageMember)
};
...
const getters = {
  ...
  currentManageMember(state) {
    return state.currentManageMember;
  }
};
...
const mutations = {
  ...
  // 当前农场
  setCurrentManageMember(state, payload) {
    let {
      manageMember
    } = payload;
    state.currentManageMember = formatData(manageMember);
  },
  updateCurrentManageMember(state, payload) {
    let {
      key,
      value
    } = payload;
    _.set(state.currentManageMember, key, value);
  },
  updateExpandedByKeyIndex(state, payload) {
    ...
    state.currentManageMember[key] = state.currentManageMember[key].map((item, _index) => {
      if (index === _index) {
  ...
  resetCurrentManageMember(state) {
    state.currentManageMember = _.cloneDeep(defaultManageMember);
  },
  // 培训操作
  addTraining(state) {
    state.currentManageMember.trainingList.push(_.cloneDeep(defaultTraining));
  },
  deleteTrainingByIndex(state, payload) {
    let {
      index
    } = payload;
    state.currentManageMember.trainingList.splice(index, 1);
  },
  // 证书操作
  addCertificate(state) {
    state.currentManageMember.certificateList.push(_.cloneDeep(defaultCertificate));
  },
  deleteCertificateByIndex(state, payload) {
    let {
      index
    } = payload;
    state.currentManageMember.certificateList.splice(index, 1);
  }
}

let isLoading = false;
const actions = {
  async createManageMember({
    state
  }, id) {
    try {
      if (!isLoading) {
        ...
        let reqBody = _.cloneDeep(Object.assign({}, serializeData(currentManageMember), {
          farmId: id
        }));
        let res = await service.createManageMember(reqBody);
        isLoading = false;
        return res;
      }
    } catch (err) {
  ...
  async updateManageMemberById({
    ...
    try {
      if (!isLoading) {
        ...
        let reqBody = Object.assign({}, serializeData(currentManageMember), {
          id
        }, {
          farmId
        });
        let res = await service.updateManageMemberById(reqBody);
        isLoading = false;
        return res;
      }
    } catch (err) {
  ...
  async deleteManageMemberById({}, id) { // eslint-disable-line
    try {
      if (!isLoading) {
        isLoading = true;
        let res = await service.deleteManageMemberById(id);
        isLoading = false;
        return res;
      }
    } catch (err) {
  ...
  async getManageMemberById({
    commit
  }, id) {
    try {
      let res = await service.getManageMemberById(id);
      if (res && res.code === 0) {
        commit('setCurrentManageMember', {
  ...
  async getManageMemberList({
    ...
    try {
      ...
      let res = await service.getManageMemberList(params);
      if (res && res.code === 0) {
        let {
          list,
          pageNum,
