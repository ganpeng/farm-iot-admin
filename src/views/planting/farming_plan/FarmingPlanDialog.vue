          ...
          >
            <el-col :span="24">
              ...
              <el-form-item label="作物周期" prop="cropCycleId">
                <el-select
                  :value="farmingPlan.cropCycleId"
                  clearable
                  placeholder="请选择"
                  @change="inputHandler($event, `cropCycleId`)"
              ...
              <el-form-item label="农事类型" prop="agriculTypeId">
                <el-select
                  :value="farmingPlan.agriculTypeId"
                  clearable
                  placeholder="请选择"
                  @change="inputHandler($event, `agriculTypeId`)"
              ...
              <el-form-item label="开始时间" prop="startAt">
                <el-date-picker
                  :value="farmingPlan.startAt"
                  type="datetime"
                  format="yyyy-MM-dd HH:mm"
                  value-format="timestamp"
                  :picker-options="datePickerOptions"
                  @input="inputHandler($event, `startAt`)"
                ></el-date-picker>
              </el-form-item>
              <el-form-item label="结束时间" prop="endAt">
                <el-date-picker
                  :value="farmingPlan.endAt"
                  type="datetime"
                  format="yyyy-MM-dd HH:mm"
                  value-format="timestamp"
                  :picker-options="datePickerOptions"
              ...
              <div v-if="farmingPlan.inputs && isComplete" class="step-content-item">
                <div class="training-list-container">
                  <ul class="training-list">
                    <li
                      v-for="(p2Plant, index) in farmingPlan.p2InputThing"
                      :key="index"
                      class="training-item"
                    >
              ...
              <div v-if="farmingPlan.products && isComplete" class="step-content-item">
                <div class="training-list-container">
                  <ul class="training-list">
                    <li
                      v-for="(p4Plant, index) in farmingPlan.p4HarvestManage"
                      :key="index"
                      class="training-item"
                    >
                      <div class="training-item-header">
            ...
            <el-col :span="24">
              <el-form-item
                class="img-form-item"
                prop="urlList"
                :rules="[{ required: isComplete, message: '该选项不能为空' }]"
                label="添加图片/视频">
                <carousel
                ...
                ></carousel>
                <uploader
                  v-if="farmingPlan.urlList.length < 9"
                  :id="`p7FileUploader`"
                  type="all"
                  :uploadSuccessHandler="fileUploadHandler()"
                ></uploader>
              </el-form-item>
              <el-form-item label="农事描述" :rules="[{ required: isComplete, message: '该选项不能为空' }]" prop="farmDes">
                <div class="desc-wrapper two">
                  <el-input
                    :value="farmingPlan.farmDes"
                    type="textarea"
                    :maxlength="300"
                    @input="inputHandler($event, 'farmDes')"
                  ></el-input>
                  <span class="desc-prompt">已输入{{farmingPlan.farmDes.length}}/300字</span>
                </div>
              </el-form-item>
              <el-form-item label="摄像头" prop="cameraId">
                <el-select
                  :value="farmingPlan.cameraId"
                  clearable
                  placeholder="请选择"
...
export default {
  ...
  methods: {
    ...
    inputHandler(value, key) {
      ...
      if (key === 'inputs') {
        if (value) {
          let p2InputThing = _.get(this.farmingPlan, `p2InputThing`) || [];
          if (p2InputThing.length === 0) {
            this.addP2InputThingHandler();
          }
        } else {
          _.set(this.farmingPlan, `p2InputThing`, []);
        }
      }
      if (key === 'products') {
        if (value) {
          let p4HarvestManage = _.get(this.farmingPlan, `p4HarvestManage`) || [];
          if (p4HarvestManage.length === 0) {
            this.addp4HarvestManageHandler();
          }
        } else {
          _.set(this.farmingPlan, `p4HarvestManage`, []);
        }
      }
      ...
      if (key === `dateRange`) {
        ...
        _.set(this.farmingPlan, `startAt`, startAt);
        _.set(this.farmingPlan, `endAt`, endAt);
      }
    },
    ...
    deleteItemHandler(key, index) {
      let keyData = _.get(this.farmingPlan, key);
    ...
    getAgriculPlanDetail() {
      ...
      this.$service.getAgriculPlanDetail(this.editId, uuid)
        .then((res) => {
          if (res && res.code === 0) {
            if (res.data !== null) {
              let {startAt, endAt, inputs, products, status} = res.data;
              let farmingPlan = _.assign({}, res.data, {
    ...
    formatReqBody() {
      ...
      let {cropCycleId, agriculTypeId} = this.farmingPlan;
      let cropCycleName = this.getCropCycleName(cropCycleId);
      let agriculTypeName = this.getAgriculTypeName(agriculTypeId);
      this.farmingPlan.cropCycleName = cropCycleName;
      this.farmingPlan.agriculTypeName = agriculTypeName;
