  ...
  <div>
    <div id="operate-block">
      ...
      <div>
        ...
        <el-button @click="exportDeviceDataExcel" class="filter-add-btn">
    ...
    >
      ...
      <el-table-column prop="totalArea" align="left" label="总数(套)">
        <template slot-scope="scope">
          <div>{{ scope.row.totalNumber }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="sensorVendor" align="left" label="传感器(套)">
        <template slot-scope="scope">
          <div class="table-item">{{ scope.row.sensorNumber }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="farmName" align="left" label="球机摄像头(套)">
        <template slot-scope="scope">
          <div class="table-item">{{ scope.row.domeCameraNumber }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="manager" align="left" label="枪机摄像头(套)">
        <template slot-scope="scope">
          <div>{{ scope.row.boxCameraNumber }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="manager" align="left" label="NVR设备(套)">
        <template slot-scope="scope">
          <div>{{ scope.row.nvrNumber }}</div>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  name: "DeviceTypeModule",
  components: { LabelTag },
  data() {
    return {
      labelList: label.deviceLabelList,
      chartDisplay: true,
      deviceTypeStatisticsList: [],
      yAxis: [
        {
          name: "套",
          position: "left",
  ...
  created() {
    this.$service
      ...
      .then((res) => {
        if (res && res.code === 0) {
          this.deviceTypeStatisticsList = res.data.deviceStatisticsList || [];
          this.initMyChart(this.deviceTypeStatisticsList);
        }
      })
      .catch((err) => {
        console.log(err);
    ...
    getDeviceTypeMetricStatistics() {
      this.$service.getDeviceTypeMetricStatistics().then((response) => {
        if (response && response.code === 0) {
          ...
          this.chartData.rows = response.data.deviceStatisticsList
            .slice(0)
            .reverse();
          let that = this;
