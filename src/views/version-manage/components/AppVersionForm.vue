...
<template>
  <div id="version-container">
    ...
    <el-form
      ...
      <el-form-item class="el-form-split-block big-margin" label-width="0">
        <el-form-item label="版本号" prop="versionNo" label-width="181px" required>
          <el-input
            v-model.trim.number="versionInfo.versionNo"
            size="medium"
            maxlength="10"
            placeholder="请输入版本号">
          </el-input>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      let checkVersionNo = (rule, value, callback) => {
      ...
      return {
        ...
        infoRules: {
          ...
          versionNo: [
            {validator: checkVersionNo, trigger: 'change'},
        ...
        this.$refs['versionInfoBasic'].validate().then((resolve, reject) => {
          if (resolve) {
            if (this.status === 'EDIT') {
              ...
              this.$service.updateAppVersion(this.versionInfo).then(response => {
                if (response && response.code === 0) {
              ...
              this.$service.createAppVersion(this.versionInfo).then(response => {
                if (response && response.code === 0) {
