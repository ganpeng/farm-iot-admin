  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="耕地面积(公顷)">
        <template slot-scope="scope">
          <div class="table-item">{{scope.row.farmLandArea}}</div>
        </template>
      </el-table-column>
      <el-table-column
        prop="farmName"
        ...
        label="宅基地面积(公顷)">
        <template slot-scope="scope">
          <div class="table-item">{{scope.row.homesteadArea}}</div>
        </template>
      </el-table-column>
      <el-table-column
        prop="manager"
    ...
    data() {
      this.chartSettings = {
        labelMap: {
          ...
          'paddyFieldArea': '水田面积',
          'dryLandArea': '旱田面积',
          'irrigationLandArea': '水浇地面积'
        },
        // stack: {
        //   '1': ['farmLandArea', 'homesteadArea'],
        //   '2': ['paddyFieldArea', 'dryLandArea', 'irrigationLandArea'],
        //   '3': ['totalArea']
        // }
      };
      return {
        ...
        landMetricDataList: [],
        chartData: {
          columns: ['year', 'totalArea', 'farmLandArea', 'homesteadArea', 'paddyFieldArea', 'dryLandArea', 'irrigationLandArea'],
          rows: []
        },
        textStyle: {
    ...
    methods: {
      ...
      exportChart() {
        ...
        canvas.toBlob((blob) => {
          FileSaver.saveAs(
            blob,
            '地块统计' + this.$util.formatDate(new Date(), 'yyyy-MM-DD_HH:mm:SS') + '.png'
          )
        })
      },
      getLandMetricStatistics() {
        this.$service.getLandMetricStatistics().then(response => {
          if (response && response.code === 0) {
            this.landMetricDataList = response.data;
            this.chartData.rows = response.data.slice(0).reverse();
            //   添加图表事件
