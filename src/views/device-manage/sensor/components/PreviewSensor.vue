<template>
  <div id="dialog-content-container">
    <div class="data-tag">共{{sensorInfoList.length}}个企业，{{previewedSensorList.length}}个传感器</div>
...
<script>
  export default {
    ...
    props: ['sensorInfoList', 'previewedSensorList'],
    mounted() {
      this.init();
    },
    methods: {
      init() {
        this.total = this.previewedSensorList.length;
      ...
      getPreviewSensorList() {
        ...
        this.currentPreviewSensorList = this.previewedSensorList.slice(
          begin,
          end
        );
      }
