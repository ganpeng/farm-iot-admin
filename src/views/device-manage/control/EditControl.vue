  ...
  <div id="sensor-container">
    ...
    <el-form
      ...
      <el-form-item class="el-form-split-block" label-width="0">
        ...
        <el-form-item label="经纬度" prop="latitude" label-width="190px">
          <div class="latitude-block">
            <label>{{controllerInfo.latitude}} {{controllerInfo.longitude}}</label>
            <span v-if="controllerInfo.latitude || controllerInfo.longitude" class="modify-btn"
      ...
      <el-form-item class="el-form-split-block big-margin" label-width="0">
        ...
        <el-form-item label="厂商" prop="controllerVendor" label-width="190px">
          {{controllerInfo.controllerVendor}}
          <!-- <div class="vendor-name">{{}}</div> -->
        </el-form-item>
      </el-form-item>
      <el-form-item class="el-form-split-block big-margin" label-width="0">
        <el-form-item label="预警模型" label-width="190px">
          <el-autocomplete
            v-model="controllerInfo.modelName"
            value-key="name"
        ...
        <el-form-item label="所属地块" prop="landCode" required label-width="190px">
          <el-select
            v-model="controllerInfo.landCode"
            size="medium"
    ...
    <el-form
      ...
      <el-form-item class="el-form-split-block big-margin" label-width="0">
        ...
        <el-form-item label="员工姓名" prop="manager" label-width="190px">
          <el-input
            v-model="controllerInfo.manager"
            size="medium"
            placeholder="员工姓名">
          </el-input>
        </el-form-item>
      ...
      <el-form-item class="el-form-split-block big-margin" label-width="0">
        <el-form-item label="员工电话" prop="managerPhone" label-width="181px">
          <el-input
            v-model="controllerInfo.managerPhone"
            size="medium"
            placeholder="员工电话">
          </el-input>
        </el-form-item>
  ...
  export default {
    ...
    data() {
      ...
      let checkLatitude = (rule, value, callback) => {
        if (this.$util.isEmpty(this.controllerInfo.latitude)) {
          callback();
        } else {
          callback();
        }
    ...
    methods: {
      handlerChangeFarm(value) {
        if (!this.controllerInfo.selectedFarm || !this.isFarmName(value)) {
          this.controllerInfo.farmId = '';
          this.controllerInfo.farmName = '';
        }
        this.controllerInfo.selectedFarm = false;
      },
      isFarmName(farmName) {
        let tag = false;
      ...
      handlerChangeModel(value) {
        if (!this.controllerInfo.selectedModel || !this.isModelName(value)) {
          this.controllerInfo.modelId = '';
          this.controllerInfo.modelName = '';
        }
        this.controllerInfo.selectedModel = false;
      },
      ...
      selectLand(landCode) {
        this.landOptions.map(item => {
          if (item.code === landCode) {
            this.controllerInfo.landName = item.name;
            this.controllerInfo.landId = item.id;
          }
        });
      },
      // 放弃修改经纬度
      ...
      // 修改经纬度
      confirmPosition() {
        this.controllerInfo.latitude = this.$refs.map.getPosition().lat;
        this.controllerInfo.longitude = this.$refs.map.getPosition().lng;
        this.selectMapPositionVisible = false;
      },
      // 获取农场列表
      ...
      handleSelectFarm(item) {
        this.controllerInfo.farmId = item.id;
        this.controllerInfo.farmName = item.name;
        this.getLandList(true);
      },
      handleSelectModel(item) {
        this.controllerInfo.modelId = item.id;
      },
      getLandList(isReset) {
        if (isReset) {
          this.controllerInfo.landCode = '';
        }
        this.landOptions = [];
        let landOptions = [];
        let homesteadPromise = this.$service.getHomesteadList({
          pageNum: 0,
          pageSize: 1000,
          farmId: this.controllerInfo.farmId
        }).then(response => {
          if (response && response.code === 0) {
      ...
      // 更新控制设备
      operateSensor() {
        Promise.all([this.$refs['controllerInfo-0'].validate(), this.$refs['controllerInfo-1'].validate(), this.$refs['controllerInfo-2'].validate()]).then(res => {
          this.$service.updateControllerInfo(this.controllerInfo).then(response => {
            if (response && response.code === 0) {
