    ...
    >
      <el-row>
        ...
        <el-col :span="12">
          ...
          <el-form-item label="企业坐标" prop="longitude">
        ...
        <el-col :span="12">
          <el-form-item label="企业类型" prop="farmType">
            <el-select
              :value="farm.farmType"
              filterable
              clearable
          ...
          <el-form-item label="企业标签" prop="farmMarks">
            <div class="my-tags">
              <el-tag
                :key="index"
        ...
        <el-col :span="12">
          <el-form-item class="img-form-item" label="企业封面" prop="farmIndexUrl">
            <el-image
              class="cover-image"
              fit="cover"
              @click="showFarmImage"
        ...
        <el-col :span="12">
          <el-form-item class="img-form-item" label="企业证书" prop="farmCertificates">
            <file-carousel
        ...
        <el-col :span="24">
          <el-form-item class="img-form-item" label="企业视频" prop="farmVideoList">
            <div class="farm-video-list-container">
              <video-carousel :deleteVideoByIndex="deleteVideoByIndex" v-if="farm.farmVideoList.length > 0" :dataList="farm.farmVideoList"></video-carousel>
              <no-data text="暂无视频" v-else></no-data>
            </div>
            <uploader id="farmVideoUploader" type="video" :uploadSuccessHandler="farmVideoUploadHandler"></uploader>
            <span class="prompt" style="margin-left: 10px;margin-bottom: 20px;">视频大小100M以内</span>
          </el-form-item>
        </el-col>
      </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      farmRules: {
        ...
        feature: [{ required: true, message: "请输入企业特色" }],
        description: [
          { required: true, message: "请输入企业简介" },
          { min: 100, max: 300, message: '企业简介长度在100~300字符之间' }
        ],
        farmType: [{ required: true, message: "请选择企业类型" }],
        // creditCode: [{ required: true, message: "请输入统一社会信用代码" }],
        manageTypes: [{ required: true, message: "请选择企业经营项目" }],
        // farmMarks: [{ required: true, message: "请选择企业标签" }],
        ...
        farmIndexUrl: [{ required: true, message: "请上传企业封面" }]
      }
    };
  },
  computed: {
  ...
  methods: {
    ...
    setAddress() {
      ...
      if (address || areaCode) {
        if (
          ...
          this.updateCurrentFarm({ key: "detailAddress", value: detailAddress });
        }
      }
    },
    setCorporationAddress() {
      let { address, areaCode, detailAddress } = this.$refs.corporationAddrSelect.getAddress();
      if (address || areaCode) {
    ...
    setContactAddress() {
      let { address, areaCode, detailAddress } = this.$refs.contactAddrSelect.getAddress();
      if (address || areaCode) {
    ...
    showFarmImage() {
      this.$refs.imagePreview.init([this.farm.farmIndexUrl], 0);
    },
    farmVideoUploadHandler(data) {
      let { fileUrl } = data;
      let farmVideoList = _.get(this.farm, `farmVideoList`);
      this.updateCurrentFarm({ key: "farmVideoList", value: [fileUrl, ...farmVideoList]});
    },
    deleteVideoByIndex(index) {
      let farmVideoList = _.get(this.farm, `farmVideoList`);
      let newFarmVideoList = farmVideoList.filter((url, _index) => index !== _index);
      this.updateCurrentFarm({ key: "farmVideoList", value: newFarmVideoList});
    }
  }
};
</script>
