...
<script>
  import VersionFilter from './components/AppVersionFilter';
  import VersionTable from './components/AppVersionTable';
  ...
  export default {
    ...
    methods: {
      init() {
        if (this.$wsCache.localStorage.get('appVersionFilter')) {
          this.listQueryParams.pageSize = this.$wsCache.localStorage.get('appVersionFilter').pageSize;
          this.listQueryParams.pageNum = this.$wsCache.localStorage.get('appVersionFilter').pageNum;
          this.listQueryParams = this.$wsCache.localStorage.get('appVersionFilter');
      ...
      getVersionList(searchParams, isReset) {
        ...
        this.$wsCache.localStorage.set('appVersionFilter', this.listQueryParams);
        this.$service.getAppVersionList(this.listQueryParams).then(response => {
      ...
      getVersionFixedTotal() {
        return this.$service.getAppVersionList({pageNum: 1, pageSize: 10}).then(response => {
          if (response && response.code === 0) {
